...
public abstract class MicroService implements Runnable {
    ...
    private String name;
    private HashMap<Class<? extends Message>, Callback> messages = new HashMap<>();
    ...
    public final void run() {
        mb.register(this);
        initialize(); //subscribe to all events and broadcast and decide the callback of what to do for each message
        try {
            while (!terminated) {
                Message msg = mb.awaitMessage(this);
                Class<? extends Message> type = msg.getClass();
                //handle the message, call the right callback by the type
                messages.get(type).call(msg);
            }
        }
